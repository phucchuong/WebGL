...
int main(int n, char **args)
{
	...
	#if defined(_WIN32) && defined( USE_MEMDBG )
		MemoryDump();
	#endif
...
LRESULT CALLBACK WndProcedure(HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_RBUTTONDOWN:		
		mouseDown = true;		
		Input::GetInstance()->OnPointer(POINTER_DOWN, x, y);
		break;

	case WM_LBUTTONUP:	
	case WM_RBUTTONUP:
		mouseDown = false;
		Input::GetInstance()->OnPointer(POINTER_UP, x, y);
		break;
	case WM_MOUSEMOVE:		
		if (mouseDown)
		{			
			Input::GetInstance()->OnPointer(POINTER_DRAGGED, x, y);
		}		
		break;
	default: 
...
bool Init(GLuint width, GLuint height)
{
	...
	AdjustWindowRectEx(&sRect, WS_CAPTION, false, 0);
	hWnd = CreateWindow( "OpenGLES2.0", "Kem", WS_VISIBLE | WS_SYSMENU,
